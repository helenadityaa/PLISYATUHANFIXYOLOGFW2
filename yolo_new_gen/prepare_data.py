...
def validate_split_ratios(train_ratio, val_ratio, test_ratio):
    ...
    invalid = [name for name, value in ratios.items() if value <= 0.0 or value >= 1.0]
    if invalid:
        raise ValueError(
            "Split ratios must be greater than 0 and less than 1: "
            + ", ".join(invalid)
        )
    ...
    if abs(total_ratio - 1.0) > 1e-6:
        raise ValueError(
            ...
            f"{total_ratio:.6f} from train={train_ratio}, val={val_ratio}, test={test_ratio}"
        )
...
def prepare_data_clean(
    ...
    train_df, val_df, test_df, group_df = prepare_group_split(
        df_filtered,
        seed=seed,
        train_ratio=train_ratio,
        val_ratio=val_ratio,
