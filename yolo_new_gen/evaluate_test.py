...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--data",
        default="yolo_new_gen/dataset_yolo_det_192_3class_vv_vh_rgb_scene_80_10_10_seed7/data.yaml",
    ...
    parser.add_argument("--device", default="0", help="Validation device, for example 0.")
    parser.add_argument("--augment", action="store_true", help="Use test-time augmentation during validation.")
    parser.add_argument("--iou", type=float, default=0.7, help="NMS IoU threshold used by Ultralytics validation.")
...
def write_args_yaml(output_path: Path, row: Dict[str, object]) -> None:
    output_path.parent.mkdir(parents=True, exist_ok=True)
    lines = [f"{key}: {format_yaml_value(value)}" for key, value in row.items()]
...
def disable_ultralytics_label_cache_writes() -> None:
    ...
    Ultralytics 8.4.x expects cache metadata such as ``version`` to exist after
    label scanning. Skipping cache writes can leave that metadata unset and
    raise KeyError during validation, so cache writes are intentionally allowed.
    """
    return
...
def main() -> None:
    ...
    write_args_yaml(save_dir / "args.yaml", args_row)
    print(f"Saved test args to {save_dir / 'args.yaml'}")
